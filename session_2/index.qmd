...
The combination of these two constraints yields the simple condition:
...
Since investment comoves one to one with productivity shock (conditionnally on $z_{t-1}$), it is way to volatile.
...
where $\omega$ is an adjustment friction. Typicall, $\omega$ is chosen so that the model replicates $\frac{Var(i_t)}{Var{y_t}}$
...
- the ergodic distribution of the model variables would not be defined.
...
General idea: introduce a force that will pull the level of foreign assets towards an equilibrium
...
- endogenous time-discount (aka Usawa preferences) (TODO: actual version has labour)
    $$\beta(c_t) = (1+c_t)^{-\chi}$$
- costs of adjustment for international portfolio

SGU show that the choice of the stationarization device has little effect
...
| Parameters | σ | ψ    |    α |     ω |    r |   δ | ρ    | σ²      | $A^{\star}$ | χ        |
|------------|---|------|------|-------|------|-----|------|---------|-------------|----------|
| Values     | 2 | 1.45 | 0.32 | 0.028 | 0.04 | 0.1 | 0.42 | 0.0129  | -0.7442     | 0.000742 |
